    ...
    return {
        ...
        make_input: function(el) {
			$('#startstop').click(function() {
				// start animation
				if ($(this).html() === "start") {
					$(this).html("stop").addClass('started');            
					
					//get values for tempo and restrict
					var tempo = parseInt($('.bpm-display').val(), 10);
					if (!tempo) { tempo = 80; }
					else if (tempo > 240) { tempo = 240; }
					else if (tempo < 80) { tempo = 80; }
					$(".bpm-display").val(tempo);

					// pass Infinity to Rapheal so it repeats until the user stops it
					m.start(tempo, "Infinity");
				} else {
					$(this).html("start").removeClass('started');
					m.stop();
				}
			});        	      	        
        }
